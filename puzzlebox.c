...
void phase02(){
  int a = atoi(next_input()); // 7
  int b = atoi(next_input()); // 7
  int c = atoi(next_input()); // 7 
  if(a==0){
    goto FAIL2;
  }
...
void phase03(){
  ...
  int targ = // 67111424  ->>>> 100000000000000101000000000
    1 << ((hash % 7)+24) |
    1 << (hash % 17)     |
    1 << (hash % 19);
...
void phase06(){
  ...
  int location1 = (hash % 211) % ilen; // 8 
  int location2 = (hash % 311) % ilen;  // 12
...
void phase09(){
  ...
  long seed = (((long) hash) << 32) + ~hash;  // 7381536182818014194
  pb_srand(seed);                           
  scramble(letters, 100);                   
  
  char buf[BUFSIZE] = {};         // KtDHu           
  for(int i=0; i<5; i++){           
